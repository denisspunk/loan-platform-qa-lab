...
Each finding from [BUGS.md](BUGS.md) and the tests that hold it in place.

| Finding | Requirement | Pinned by | State |
|---|---|---|---|
| F-01 payment to a paid-off loan is dropped | BR-11 | `PaymentsApiTest#paymentToAPaidOffLoanChangesNothing`, `LoanHistoryApiTest#paymentToAPaidOffLoanIsRecordedAsAlreadyPaidOff`, `RemoteApiTest#paymentToAPaidOffLoanChangesNothing`, `#paymentHistoryShowsBothResults` | Partly addressed: recorded in Postgres, money handling awaits product |
| F-02 overpayment is not tracked | BR-12 | `LoanTest#overpaymentShowsZeroBalance` | Open |
| F-03 relock not cancelled on payoff | INT-08 | — | Open, **no test** |
| F-04 partner failure leaves the phone unlocked | INT-07 | `PaymentConsumerTest` and `PaymentsApiTest`, each a passing test of today's behaviour plus a `@Disabled` test of the expected one | Open, reproduced |
| F-05 duplicate protection lost on restart | DATA-03 | `JdbcLoanRepositoryTest#samePaymentRecordedTwiceIsNotAnError` | Fixed, restart itself untested |
| F-06 repeated unlock | INT-09 | — | Open, **no test** |
| F-07 `relockAt` carries microseconds | INT-03 | `KnoxClientContractTest#relockKeepsFractionsOfASecond` | Open, question to partner |
| F-08 empty loan id gives 404 | API-11 | `LoansApiTest#emptyLoanIdIsAnsweredAsAnUnknownLoan` | Open |
| F-09 `receivedAt` ignored | BR-13 | — | Open, **no test** |
| F-10 HTML 404 outside the API | API-14 | `HealthAndRoutingTest#pathOutsideTheApiIsRefusedWithAJsonError` | Fixed, the disabled test was switched on |
| F-11 unknown fields ignored | API-12 | `LoansApiTest#unknownFieldsAreIgnored`, `#loanWithAMissingOrInvalidFieldIsRejected` (the `no dailyRate` case) | Open, question to API contract |
| F-12 parser details in errors | API-09 | `LoansApiTest#brokenJsonErrorDoesNotExposeParserDetails` (`@Disabled`) | Open |
